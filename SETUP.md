...
Boss of the SOC v3 (BOTS) is a real, public (CC0) Splunk dataset (`aws:cloudtrail`, `syslog`,
network streams; ~2.08M events / ~320 MB compressed). No Splunk account required - direct
public download.

Both ARGUS scenarios are scoped to one real, well-documented incident inside BOTS v3: IAM user
**`web_admin`** spraying `RunInstances` (cryptomining) across 10 AWS regions on 2018-08-20 - 576
`aws:cloudtrail` events, all inside a single hour (2018-08-20 09:00 UTC): 280
`Client.InstanceLimitExceeded` + 185 `Client.UnauthorizedOperation` + 104 `Client.Unsupported` +
7 `Server.InsufficientInstanceCapacity` - i.e. **576 launches in one hour from one identity**,
versus near-zero for normal `AssumedRole` autoscaling. Both scenarios' baseline detections and
the anomaly-scorer baseline are computed live from this incident.

```bash
# Public dataset (~320 MB compressed, public S3, CC0). One-time download.
...
> If an empty `botsv3` index already exists (e.g. created by the Step 3 REST call below, before
> this data was loaded), remove its `[botsv3]` stanza from `etc/apps/search/local/indexes.conf`
> and restart again - a `local/` override of the same index name shadows the dataset app's
> `default/indexes.conf` (which points at the real bucket data) with the empty one.
...
curl -sk -u admin:ChangeMe_Strong123! \
  --data-urlencode "search=search index=botsv3 sourcetype=aws:cloudtrail eventName=RunInstances userIdentity.userName=web_admin earliest=0 | stats count" \
...
# result.count should be 576 - the real incident both scenarios' baseline detections and the
# anomaly-scorer baseline are computed from.
```
...
|---|---|---|---|
| `SPLUNK_HOST` | Yes | `127.0.0.1` | Use `127.0.0.1`, not `localhost` (see troubleshooting) |
| `SPLUNK_PORT` | Yes | `8089` | Splunk management port |
| `SPLUNK_SCHEME` | | `https` | |
| `SPLUNK_USERNAME` | Yes | `admin` | |
| `SPLUNK_PASSWORD` | Yes | - | |
...
| `SPLUNK_HEC_URL` | Yes | `https://127.0.0.1:8088/services/collector/event` | |
| `SPLUNK_HEC_TOKEN` | Yes | - | Created in Step 4 |
