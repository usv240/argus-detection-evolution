...
- **Real-attack validation** — does the evolved rule catch the genuine attack in the BOTS dataset? (✓ or ✗, computed live)
...
│
├── backend/
│   ├── api.py                      FastAPI: /api/arena (SSE) /api/health /api/scenarios /api/approval
...
| **Coverage headline** | Big `0% → X%` number, false positives, real-attack ✓/✗, search count + provider |
| **Judge Proof panel** | All measurable facts in one block — coverage, FP, variants, real-attack, searches, index, run ID, cert SHA-256 |
| **Generation cards** | Per-round: recall before/after, Red's evasions (name · MITRE · changed fields · why baseline missed · caught/evaded), Blue's rationale |
| **Baseline → Evolved SPL** | Side-by-side, with green lines highlighting what Blue added, and "why it catches" |
...
| **Residual frontier** | Evasions still uncaught; labeled as prioritized blind spots |
| **Resilience Certificate** | Run ID, before/after, SHA-256 fingerprint, download button |
...
# Load BOTS v3 (public CC0, ~320 MB)
...
show `splunk-mcp` as the provider for every live query.
...
Or manually:
1. `GET http://127.0.0.1:8810/api/health` → `splunk.connected: true`, `llm_configured: true`, `hec_configured: true`
2. `GET http://127.0.0.1:8810/api/scenarios` → returns 2 scenarios
3. Open http://127.0.0.1:5180 → status dots green → **Launch the Arena** → **Run the Arena**
...
[MIT](LICENSE) · Built by Ujwal Suresh Vanjare for the Splunk Agentic Ops Hackathon, June 2026.
